...
def run_sweep(n_files=50, rows_per_file=10):
    ...
    for i, src_path in enumerate(files):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            pl.scan_parquet(src_path).head(rows_per_file).select("claims").collect().write_parquet(input_path)
    ...
    return pl.DataFrame(measurements).with_columns(pl.lit(rows_per_file).alias("rows_per_file"))

def main():
    # Sweep schedule: 1,2,3,4,5 then 10,15,20,...,100
    row_counts = [1, 2, 3, 4, 5] + list(range(10, 101, 5))
