...
def _check_all_targets(
    ...
    for table in Table:
        ...
        if not repo_id:
            ...
            return pl.repeat(False, n=len(files), eager=True).alias("already_pushed")
    ...
    in_all = pl.all_horizontal(Table).alias("already_pushed")
    return result.select(in_all).get_column("already_pushed")
